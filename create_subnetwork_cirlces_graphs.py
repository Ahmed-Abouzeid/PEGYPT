...
data_f = open('sub_network_data.pkl', 'rb')
...
sub_users_f = open('sub_network_users.pkl', 'rb')
sub_users = pickle.load(sub_users_f)
...
def create_colored_graphs():
    ...
    node_bias = open('./graphs/subnetwork/circles_alone/bias_nodes.csv', 'a')
    edge_bias = open('./graphs/subnetwork/circles_alone/bias_edges.csv', 'a')
    node_propaganda = open('./graphs/subnetwork/circles_alone/propaganda_nodes.csv', 'a')
    edge_propaganda = open('./graphs/subnetwork/circles_alone/propaganda_edges.csv', 'a')
            ...
            if str(user_id) in circle_A:
                if str(user_id) in users_bias_for and str(user_id) not in users_bias_against and str(user_id)  not in users_neutral_bias:
                    ...
                    for index, entry in enumerate(adj[e]):
                        if entry > 0 and str(users[index]) in circle_A and users[index] in sub_users:
