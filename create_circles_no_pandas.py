...
def is_before(days, hours, minutes, seconds):
    if days[0] < days[1]:
        return True
    elif days[0] == days[1] and hours[0] < hours[1]:
        return True
    elif days[0] == days[1] and hours[0] == hours[1] and minutes[0] < minutes[1]:
        return True
    elif days[0] == days[1] and hours[0] == hours[1] and minutes[0] == minutes[1] and seconds[0] < seconds[1]:
        return True
    else:
        return False
...
def get_temporal_circle(current_time_bound, user_id, tweets_with_authors, network_user_ids,  tweet_to_tweet_engagements, tweet_to_user_engagement, all_feed):
    ...
    for t_id, user_ids in tweet_to_user_engagement.items():
        if user_id in user_ids:
            ...
            if mentioning_u_id != user_id and time_stamp is not None:
                ...
                if is_before([dy0, dy1], [hr0, hr1], [min0, min1], [sc0, sc1]):
                    bias = all_feed[t_id][0]
                    is_propaganda = all_feed[t_id][1]
                    circles.append((bias, is_propaganda, None))
    ...
    for t_id, t_ids in tweet_to_tweet_engagements.items():
        ...
        if citing_author_id == user_id and time_stamp is not None and is_before([dy0, dy1], [hr0, hr1], [min0, min1], [sc0, sc1]):
            for cited_tweet_id in t_ids:
                ...
                if cited_author_id != user_id:
                    ...
                    circles.append((bias, is_propaganda, all_feed[t_id][0] == bias and all_feed[t_id][1] == is_propaganda))
        ...
        for cited_tweet_id in t_ids:
            ...
            if citing_author_id != user_id and cited_author_id == user_id and time_stamp is not None and cited_tweet_id not in matched:
                ...
                if is_before([dy0, dy1], [hr0, hr1], [min0, min1], [sc0, sc1]):
                    ...
                    bias = all_feed[t_id][0]
                    is_propaganda = all_feed[t_id][1]
                    circles.append((bias, is_propaganda,  all_feed[cited_tweet_id][0] == bias and all_feed[cited_tweet_id][1] == is_propaganda))
...
for e, record in enumerate(all_feed_labels):
    ...
    #print(u_id, time_stamp, record['bias'], record['propaganda'], temporal_circles)
    citing_temporal_circles_network.update({record['id']: [time_stamp, u_id, record['bias'], record['propaganda'], temporal_circles]})
temp_pkl_f = open('temporal_circles_acceptance.pkl', 'wb')
