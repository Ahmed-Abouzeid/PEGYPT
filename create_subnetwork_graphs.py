...
def create_colored_graphs():
    ...
    node_bias = open('./graphs/subnetwork/bias_nodes.csv', 'a')
    edge_bias = open('./graphs/subnetwork/bias_edges.csv', 'a')
    node_propaganda = open('./graphs/subnetwork/propaganda_nodes.csv', 'a')
    edge_propaganda = open('./graphs/subnetwork/propaganda_edges.csv', 'a')
        ...
        if user_id in sub_users:
            if str(user_id) in users_bias_for and str(user_id) not in users_bias_against and str(user_id)  not in users_neutral_bias:
                ...
                for index, entry in enumerate(adj[e]):
                    if entry > 0 and users[index] in sub_users:
